...
let numerosDecrescentes = numeros.sorted(decrescente)
...
func executarClosureApos(#tempo: UInt32, closure: () -> ()) {
    ...
    println("Aguardando \(tempo)s")
    sleep(tempo)
    closure()
}

// Passando a closure DENTRO dos parênteses
executarClosureApos(tempo: 1, {
    println("Dentro")
})
...
func criarIncrementador(valorIncremento: Int) -> () -> Int {
